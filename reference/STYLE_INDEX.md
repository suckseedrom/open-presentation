...
- `reference/PRODUCT_PILLARS.md` — delivery promises and output rules
- `reference/STYLE_GUIDE.md` — house-style contract shared across the pack
- `reference/scene-grammar.json` — shared scene sequencing and pacing rules
...
| `capsule` | You want modular, rounded, friendly surfaces with premium clarity | Rounded cards, modular comparisons, upbeat clarity | You need institutional seriousness or hard-edge austerity |
| `cobalt-grid` | You want design-research precision with sharp typographic structure | Crisp grid geometry, analytical rhythm, high-structure layout | You need warmth, softness, or playful motion |

## Loading rule

1. Read `templates/index.json`.
2. Shortlist up to 3 candidate templates from metadata and preview cues only.
3. Read only those templates' `preview.md` files.
4. After the user chooses, read exactly one `design.md`.
5. Use template-local example briefs only for the selected default/base template when they help the current job.
